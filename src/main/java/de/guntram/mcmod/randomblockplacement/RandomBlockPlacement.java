...
public class RandomBlockPlacement implements ClientModInitializer, ClientCommandPlugin
{
    ...
    public void setActive() {
        isActive = true;
        MinecraftClient.getInstance().player.addChatMessage(new TranslatableText("msg.active", minSlot+1, maxSlot+1), false);
    }
    ...
    public void onPlayerInteract() {
        ...
        if (isActive && index>=minSlot && index<=maxSlot
            && ( inventory.getInvStack(index).getItem() == Items.AIR
                || inventory.getInvStack(index).getItem() instanceof BlockItem)) {
            int totalBlocks=0;
    ...
    private int getBlockCount(PlayerInventory inventory, int targetSlot) {
        ItemStack stack = inventory.getInvStack(targetSlot);
